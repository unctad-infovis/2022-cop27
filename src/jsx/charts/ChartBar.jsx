...
function BarChart({
  data, data_decimals, export_title_margin = 0, idx, labels_inside = false, prefix = '', note = false, source, subtitle = false, suffix = '', title, xlabel = '', ylabel = '', ymax = undefined, ymin = undefined
}) {
  ...
  const createChart = useCallback(() => {
    Highcharts.chart(`chartIdx${idx}`, {
      ...
      chart: {
        events: {
          load() {
            ...
            this.renderer.image(`${window.location.href.includes('unctad.org') ? 'https://storage.unctad.org/2022-cop27/' : './'}assets/img/unctad_logo.svg`, 5, 15, 80, 100).add();
          }
        },
        height: chartHeight,
        resetZoomButton: {
      ...
      xAxis: {
        ...
        labels: {
          formatter: (el) => ((el.value === 'Mitigation' || el.value === 'Adaptation') ? `<span class="x_axis_heading">${el.value} </span>` : (el.value !== '""') ? el.value : null),
          rotation: 0,
          style: {
            color: 'rgba(0, 0, 0, 0.8)',
            fontFamily: 'Roboto',
